...
describe("extract command", () => {
  it("runs end-to-end against a Spring fixture", async () => {
    ...
    const { stdout, stderr, exitCode } = await runCli([
      ...
      "spring",
      "--output",
      toCliPath(outputPath)
    ]);

    expect(exitCode).toBe(0);
    expect(stdout).toContain("Parsed 1 endpoints");
    expect(stderr).not.toContain("Parsed 1 endpoints");
    const collection = JSON.parse(await readFile(outputPath, "utf8")) as {
      item: { item: unknown[] }[];
    };
...
async function prepareFixture(): Promise<void> {
  ...
  await writeFile(
    join(fixtureRoot, "src", "main", "java", "com", "example", "UserController.java"),
    `
package com.example;
